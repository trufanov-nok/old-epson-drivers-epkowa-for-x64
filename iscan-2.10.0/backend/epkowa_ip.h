 ...
 */
typedef Epson_Device  device_type;
...
struct interpreter_type
{
  ...
  ssize_t (*recv) (device_type *device, void *buffer, size_t size,
		   SANE_Status *status);
  ssize_t (*send) (device_type *device, void *buffer, size_t size,
		   SANE_Status *status);
  ...
  SANE_Status (*ftor1) (device_type *device, SANE_Parameters *params,
			int depth, int left, int resolution);
...
/*! Checks for connected interpreter-supported devices.
 */
void find_interpreted_devices (SANE_Status (*attach) (const char *dev));
