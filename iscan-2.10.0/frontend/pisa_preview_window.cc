...
/*--------------------------------------------------------------*/
void preview_window::start_preview ( pisa_preview_type type )
{
  scan_manager		* scan_mgr;
  ...
  try
    {
      scan_mgr->init_scan (&param, &width, &height);

      if ( type == PISA_PREV_WHOLE )
	{
...
long preview_window::get_preview_resolution ( const _rectD * img_rect )
{
  ...
  if ( img_rect )
    ...
    {
      pt_scan_size.x = ::g_view_manager->m_set.max_area [ 0 ];
      pt_scan_size.y = ::g_view_manager->m_set.max_area [ 1 ];
...
/*--------------------------------------------------------------*/
void preview_window::reset_settings ( pisa_preview_type type )
{
  settings	* set;
  long		i, n, m, marq_num;
  marquee	* marq;
  ...
  gamma_cls = ( gamma_correction * ) ::g_view_manager->get_window_cls ( ID_WINDOW_GAMMA );
  ...
  for ( i = 0; i < marq_num; i++ )
      ...
      for ( n = 0; n < 3; n++ )
	{
      ...
      iscan::build_LUT ( set, marq, scan_info->dumb );
    }
}

/*--------------------------------------------------------------*/
int preview_window::set_preview_param ( scan_parameter * param,
					pisa_preview_type type )
{
  settings	* set;
  ...
  set = & ::g_view_manager->m_set;
  cur_marq = set->get_current_marquee ( );
  ...
  // image type
  param->pixeltype	= PISA_PT_RGB;
  param->bitdepth	= PISA_BD_8;
  param->scanspeed	= PISA_SS_DRAFT;
  param->dropout	= PISA_DO_NONE;
  param->monoopt	= PISA_MO_NONE;
  param->halftone	= PISA_HT_NONE;

  // gamma table
  for ( i = 0; i < 256; i++ )
    {
      param->gamma.gamma_r [ i ]	= i;
      param->gamma.gamma_g [ i ]	= i;
      param->gamma.gamma_b [ i ]	= i;
    }
  ...
  if ( type == PISA_PREV_WHOLE )
    {
      param->offset [ 0 ]	= 0.0;
      param->offset [ 1 ]	= 0.0;
      param->area [ 0 ]		= set->max_area [ 0 ];
      param->area [ 1 ]		= set->max_area [ 1 ];
    }
  else
    {
      float rate = 0.1f;
      ...
      param->offset [ 0 ]	= pt_offset.x;
      param->offset [ 1 ]	= pt_offset.y;
      param->area [ 0 ]		= pt_area.x;
      param->area [ 1 ]		= pt_area.y;      
    }

  // resolution scale
  img_rect.left		= param->offset [ 0 ];
  img_rect.top		= param->offset [ 1 ];
  img_rect.right	= param->offset [ 0 ] + param->area [ 0 ];
  img_rect.bottom	= param->offset [ 1 ] + param->area [ 1 ];
  resolution = get_preview_resolution ( & img_rect );

  param->resolution [ 0 ]	= resolution;
...
/*--------------------------------------------------------------*/
void preview_window::zoom_boundary ( _pointD * pt_offset,
				     _pointD * pt_area,
				     _pointD & pt_max,
				     float rate )
{
...
void preview_window::modify_max_val ( void )
{
  ...
  whole_marq = ::g_view_manager->m_set.get_marquee ( 0 );
...
gint preview_window::mouse_down ( GdkEvent * event )
{
  ...
  id_cur_marquee = ::g_view_manager->m_set.get_marquee_size ( ) - 1;
  get_marquee_point ( id_cur_marquee, & pt_lefttop, & pt_rightbottom );

  cur_marq = ::g_view_manager->m_set.get_current_marquee ( );
...
gint preview_window::mouse_up ( GdkEvent * event )
{
  ...
  move = ( m_pt_begin.x != pt_mouse.x ) || ( m_pt_begin.y != pt_mouse.y );
...
int preview_window::get_marquee_point ( long i, _pointL * pt_lt, _pointL * pt_rb )
{
  ...
  if ( 0 == ( marq = ::g_view_manager->m_set.get_marquee ( i ) ) )
    return 0;
